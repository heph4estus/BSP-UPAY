...
import com.psi.tariff.plans.m.NewSendAml;
...
public class NewSendAmlCommand extends UICommand{
	...
	public IView execute() {
		...
		try {
			...
			if(sess.exists()) {
				...
				if(reg.exist()){
					reg.setState(new ObjectState("01", "Aml Send Setting already registered"));
...
//				    audit.setData(key+"|"+minamount+"|"+maxamount+"|"+maxamountday+"|"+maxamountday+"|"+maxamountweek+"|"+maxamountmonth+"|"+maxtransday+"|"+maxtransweek+"|"+maxtransmonth);
				...
				reg.setState(new ObjectState("00", "Send AML Setting Added Succesfully"));
				AuditTrail audit  = new AuditTrail();
	    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
	    		audit.setModuleid(String.valueOf(this.getId()));
	    		audit.setEntityid(key);
