...
import com.psi.tariff.plans.m.ManageSendAml;
...
public class EditAmlSendCommand extends UICommand{
	...
	public IView execute() {
		...
		try {
			...
			if(sess.exists()) {
				...
				ManageSendAml create = new ManageSendAml();
				...
				create.setAuthorizedSession(sess);
					...
					try {
						if(!create.exist()){
							create.setState(new ObjectState("01", "AML Send Doesn't Exist"));
						...
						if(create.existalready()){
							create.setState(new ObjectState("02", "AML Send Already Exist"));
							AuditTrail audit  = new AuditTrail();
				    		audit.setIp(create.getAuthorizedSession().getIpAddress());
				    		audit.setModuleid(String.valueOf(this.getId()));
				    		audit.setEntityid(type);
						...
						if(create.update()){
								create.setState(new ObjectState("00", "Successfully updated AML Send"));
