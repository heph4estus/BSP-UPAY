...
import com.psi.revenue.assurance.m.TransactionSummaryCollection;
...
public class TransactionSummaryColCommand extends UICommand{
	...
	public IView execute() {
		...
		try {
			...
			if(sess.exists()) {	
				...
				TransactionSummaryCollection model = new TransactionSummaryCollection();
				...
				model.setAuthorizedSession(sess);
					if(model.hasRows()){
			    		...
			    		audit.setEntityid("");
			    		audit.setLog("Successfully fetched transaction summary data");
			    		audit.setStatus("00");
						...
						return new CollectionView("00",model);  
							...
							ObjectState state = new ObjectState("01", "No transaction summary data found");
							AuditTrail audit  = new AuditTrail();
				    		audit.setIp(model.getAuthorizedSession().getIpAddress());
				    		audit.setModuleid(String.valueOf(this.getId()));
				    		audit.setEntityid("");
				    		audit.setLog("No transaction summary data found");
				    		audit.setStatus("01");
				    		audit.setUserid(model.getAuthorizedSession().getAccount().getId());
				    		audit.setUsername(model.getAuthorizedSession().getAccount().getUserName());
				    		audit.setSessionid(model.getAuthorizedSession().getId());
