...
import com.psi.revenue.assurance.m.AccountStatementCollection;
...
public class AccountStatementColCommand extends UICommand{
	...
	public IView execute() {
		...
		try {
			...
			if(sess.exists()) {	
				...
				AccountStatementCollection model = new AccountStatementCollection();
				...
				model.setAuthorizedSession(sess);
					if(model.hasRows()){
			    		...
			    		audit.setEntityid("");
			    		audit.setLog("Successfully fetched account statement data");
			    		audit.setStatus("00");
						...
						return new CollectionView("00",model);  
							...
							ObjectState state = new ObjectState("01", "No account statement data found");
							AuditTrail audit  = new AuditTrail();
				    		audit.setIp(model.getAuthorizedSession().getIpAddress());
				    		audit.setModuleid(String.valueOf(this.getId()));
				    		audit.setEntityid("");
				    		audit.setLog("No account statement data found");
				    		audit.setStatus("01");
				    		audit.setUserid(model.getAuthorizedSession().getAccount().getId());
				    		audit.setUsername(model.getAuthorizedSession().getAccount().getUserName());
				    		audit.setSessionid(model.getAuthorizedSession().getId());
