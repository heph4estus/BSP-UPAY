...
public class BranchByCityColCommand extends UICommand{
			...
			if(sess.exists()) {	
				...
				model.setAuthorizedSession(sess);
					if(model.hasRows()){
			    		...
			    		audit.setEntityid("");
			    		audit.setLog("Successfully fetched merchant by city data");
			    		audit.setStatus("00");
						...
						return new CollectionView("00",model);  
							...
							ObjectState state = new ObjectState("01", "No merchant by city data found");
							AuditTrail audit  = new AuditTrail();
				    		audit.setIp(model.getAuthorizedSession().getIpAddress());
				    		audit.setModuleid(String.valueOf(this.getId()));
				    		audit.setEntityid("");
				    		audit.setLog("No merchant by city data found");
				    		audit.setStatus("01");
				    		audit.setUserid(model.getAuthorizedSession().getAccount().getId());
				    		audit.setUsername(model.getAuthorizedSession().getAccount().getUserName());
				    		audit.setSessionid(model.getAuthorizedSession().getId());
