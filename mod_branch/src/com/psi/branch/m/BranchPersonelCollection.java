...
public class BranchPersonelCollection extends ModelCollection{
	...
	public boolean hasRows() {
		...
		DataRowCollection r = SystemInfo.getDb().QueryDataRows("SELECT * FROM TBLUSERS WHERE ACCOUNTNUMBER=? AND USERSLEVEL='MANAGER'",rr.getString("ACCOUNTNUMBER"));
	     if (!r.isEmpty())
	     {
	    	 for(DataRow row: r){
...
//			 	      }	
	...
	public boolean hasRowsCashier() {
		DataRow rr = SystemInfo.getDb().QueryDataRow("SELECT * FROM TBLBRANCHES WHERE ID=?",this.id);
		DataRowCollection r = SystemInfo.getDb().QueryDataRows("SELECT U.*,M.TPAID FROM TBLUSERS U LEFT JOIN TBLMACBINDING M ON M.USERNAME = U.USERNAME WHERE U.ACCOUNTNUMBER=? AND U.USERSLEVEL='CASHIER'",rr.getString("ACCOUNTNUMBER"));
	     if (!r.isEmpty())
	     {
	    	 for(DataRow row: r){
		         ReportItem m = new ReportItem();
