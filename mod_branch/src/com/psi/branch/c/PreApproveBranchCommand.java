...
public class PreApproveBranchCommand extends UICommand{
		...
		try {
			...
			if(sess.exists()) {
				...
				reg.setAuthorizedSession(sess);
						...
						if(reg.preapprove()){
							EmailUtils.sendPreApprove(SystemInfo.getDb().QueryScalar("SELECT BUSINESS FROM TBLBUSINESS WHERE ACCOUNTNUMBER IN (SELECT BUSINESSACCOUNT FROM TBLMERCHANTPNDG WHERE ACCOUNTNUMBER = ?)", "",accountnumber),reg.getBranchname(),reg.getAuthorizedSession().getAccount().getUsername());							
							reg.setState(new ObjectState("00", "Successfully pre-approved merchant outlet"));
							AuditTrail audit  = new AuditTrail();
				    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
				    		audit.setModuleid(String.valueOf(this.getId()));
				    		audit.setEntityid(accountnumber);
							...
							reg.setState(new ObjectState("01", "Failed! Unable to pre-approve merchant outlet"));
							AuditTrail audit  = new AuditTrail();
				    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
				    		audit.setModuleid(String.valueOf(this.getId()));
				    		audit.setEntityid(accountnumber);
