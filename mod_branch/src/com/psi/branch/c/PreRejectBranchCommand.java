...
public class PreRejectBranchCommand extends UICommand{
		...
		try {
			...
			if(sess.exists()) {
				...
				reg.setAuthorizedSession(sess);
						...
						if(reg.prereject()){
							EmailUtils.sendPreReject(SystemInfo.getDb().QueryScalar("SELECT BUSINESS FROM TBLBUSINESS WHERE ACCOUNTNUMBER IN (SELECT BUSINESSACCOUNT FROM TBLMERCHANTPNDG WHERE ACCOUNTNUMBER = ?)", "",accountnumber),reg.getBranchname(),reg.getAuthorizedSession().getAccount().getUsername());
							reg.setState(new ObjectState("00", "Successfully escalated merchant outlet"));
							AuditTrail audit  = new AuditTrail();
				    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
				    		audit.setModuleid(String.valueOf(this.getId()));
				    		audit.setEntityid(accountnumber);
							...
							reg.setState(new ObjectState("01", "Failed! Unable to escalate merchant outlet"));
							AuditTrail audit  = new AuditTrail();
				    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
				    		audit.setModuleid(String.valueOf(this.getId()));
				    		audit.setEntityid(accountnumber);
