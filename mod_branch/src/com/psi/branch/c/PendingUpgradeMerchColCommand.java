...
public class PendingUpgradeMerchColCommand extends UICommand{
		...
		try {
			...
			if(sess.exists()) {
				...
				if(col.hasRowsPndgUpgrade()){
		    		...
		    		audit.setEntityid("");
		    		audit.setLog("Successfully fetched pending upgrade branch data");
		    		audit.setStatus("00");
					...
					return new CollectionView("00",col);  
						...
						ObjectState state = new ObjectState("01", "No pending upgrade branch data found");
						AuditTrail audit  = new AuditTrail();
			    		audit.setIp(col.getAuthorizedSession().getIpAddress());
			    		audit.setModuleid(String.valueOf(this.getId()));
			    		audit.setEntityid("");
	...
	public String getKey() {
		return "PNDGUPGRADEBRANCHCOLLECTION";
	}
