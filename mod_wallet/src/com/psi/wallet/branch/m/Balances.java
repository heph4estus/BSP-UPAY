...
public class Balances extends Model {
	...
	public boolean paymentbalance(String accountnumber) throws ParseException, Exception {

		DataRow row = SystemInfo.getDb().QueryDataRow("SELECT DECRYPT(AMOUNT,?,ACCOUNTNUMBER) AMOUNT FROM ADMDBMC.TBLCURRENTSTOCK WHERE ACCOUNTNUMBER = ? AND WALLETID = -2",SystemInfo.getDb().getCrypt(),accountnumber);
		if(!row.isEmpty()){
				this.setCurrentbal(LongUtil.toString(Long.parseLong(row.getString("AMOUNT").toString())));
				this.setState(new ObjectState("00","Success"));  
				return true;   
		}else{
				this.setState(new ObjectState("99","System is busy"));
				 return false;
