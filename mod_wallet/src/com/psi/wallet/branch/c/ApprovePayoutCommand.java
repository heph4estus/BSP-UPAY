...
public class ApprovePayoutCommand extends UICommand{
			    ...
			    topup.setAuthorizedSession(sess);
					try {
						if(!topup.validate()){
						    ...
						    audit.setData(id+"|"+requestby+"|"+amount);
				    		audit.insert();
							return new JsonView(topup); 
						}
						if(topup.approved()){
							if(topup.sendMail()){
								topup.setState(new ObjectState("0", "Successfully approved payout request"));
								AuditTrail audit  = new AuditTrail();
					    		audit.setIp(topup.getAuthorizedSession().getIpAddress());
					    		audit.setModuleid(String.valueOf(this.getId()));
					    		audit.setEntityid(id);
