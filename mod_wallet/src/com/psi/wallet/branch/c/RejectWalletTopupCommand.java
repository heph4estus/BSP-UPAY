...
import com.psi.wallet.branch.m.RejectWalletTopUp;
				...
				topup.getData();
					try {
						if(!topup.validate()){
				    		...
				    		audit.setData(topup.getAuditdata()+"|"+remarks);
						...
						if(topup.reject()){
							...
							topup.setState(new ObjectState("0", "Successfully rejected allocation request"));
							AuditTrail audit  = new AuditTrail();
				    		audit.setIp(topup.getAuthorizedSession().getIpAddress());
				    		audit.setModuleid(String.valueOf(this.getId()));
				    		audit.setEntityid(id);
