...
import com.psi.service.fee.m.NewSFByProduct;
...
public class NewSFByProductCommand extends UICommand{
	...
	public IView execute() {
		...
		try {
			...
			if(sess.exists()) {
				...
				NewSFByProduct create = new NewSFByProduct();
				create.setModuleid(moduleid);
				...
				create.setAuthorizedSession(sess);
					...
					try {
						if(create.exist()){
							create.setState(new ObjectState("01", "Service Fee Already Exist"));
						    ...
						    audit.setData(type+"|"+moduleid+"|"+minamount+"|"+maxamount+"|"+fixed+"|"+percentage);
				    		audit.insert();
							return new JsonView(create); 

						}
						if(create.create()){
								create.setState(new ObjectState("00", "Successfully added new Service Fee"));
								AuditTrail audit  = new AuditTrail();
					    		audit.setIp(create.getAuthorizedSession().getIpAddress());
					    		audit.setModuleid(String.valueOf(this.getId()));
					    		audit.setEntityid(type);
