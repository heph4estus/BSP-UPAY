...
public class BillsPayCollection extends ModelCollection {
	...
	public boolean hasRowsAllBranch() {
		DataRowCollection r = SystemInfo.getDb().QueryDataRows("SELECT * FROM TBLTRANSACTIONSBILLSPAYMENT WHERE ACCOUNTNUMBER IN (SELECT ACCOUNTNUMBER FROM ADMDBMC.TBLACCOUNTINFO WHERE TYPE = ENCRYPT('dealer',?,ACCOUNTNUMBER)) AND TO_CHAR(TIMESTAMP,'YYYY-MM-DD') BETWEEN ? AND ?",SystemInfo.getDb().getCrypt(),this.datefrom,this.dateto);
	...
	public boolean hasRowsAllCustomer() {
		DataRowCollection r = SystemInfo.getDb().QueryDataRows("SELECT * FROM TBLTRANSACTIONSBILLSPAYMENT WHERE ACCOUNTNUMBER IN (SELECT ACCOUNTNUMBER FROM ADMDBMC.TBLACCOUNTINFO WHERE TYPE = ENCRYPT('subscriber',?,ACCOUNTNUMBER)) AND TO_CHAR(TIMESTAMP,'YYYY-MM-DD') BETWEEN ? AND ?",SystemInfo.getDb().getCrypt(),this.datefrom,this.dateto);
	...
	public boolean getBPColBranches() {
		DataRowCollection r = SystemInfo.getDb().QueryDataRows("SELECT * FROM TBLTRANSACTIONSBILLSPAYMENT WHERE ACCOUNTNUMBER=? AND TO_CHAR(TIMESTAMP,'YYYY-MM-DD') BETWEEN ? AND ?",this.accountnumber,this.datefrom,this.dateto);
