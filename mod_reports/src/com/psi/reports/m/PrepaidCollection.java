...
public class PrepaidCollection extends ModelCollection {
	...
	public boolean hasRowsAllBranch() {
		DataRowCollection r = SystemInfo.getDb().QueryDataRows("SELECT * FROM TBLTRANSACTIONSPREPAIDLOAD WHERE ACCOUNTNUMBER IN (SELECT ACCOUNTNUMBER FROM ADMDBMC.TBLACCOUNTINFO WHERE TYPE = ENCRYPT('dealer',?,ACCOUNTNUMBER)) AND TO_CHAR(TIMESTAMP,'YYYY-MM-DD') BETWEEN ? AND ?",SystemInfo.getDb().getCrypt(),this.datefrom,this.dateto);
	...
	public boolean hasRowsAllCustomer() {
		DataRowCollection r = SystemInfo.getDb().QueryDataRows("SELECT * FROM TBLTRANSACTIONSPREPAIDLOAD WHERE ACCOUNTNUMBER IN (SELECT ACCOUNTNUMBER FROM ADMDBMC.TBLACCOUNTINFO WHERE TYPE = ENCRYPT('subscriber',?,ACCOUNTNUMBER)) AND TO_CHAR(TIMESTAMP,'YYYY-MM-DD') BETWEEN ? AND ?",SystemInfo.getDb().getCrypt(),this.datefrom,this.dateto);
	...
	public boolean getCompanyCol() {
		DataRowCollection r = SystemInfo.getDb().QueryDataRows("SELECT * FROM TBLTRANSACTIONSPREPAIDLOAD WHERE ACCOUNTNUMBER=? AND TO_CHAR(TIMESTAMP,'YYYY-MM-DD') BETWEEN ? AND ?",this.company,this.datefrom,this.dateto);
