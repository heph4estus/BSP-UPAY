...
public class FailedTransAdminCollection extends ModelCollection{
	...
	public boolean hasRows() {
		DataRowCollection r = SystemInfo.getDb().QueryDataRows("SELECT * FROM TBLFAILEDTRANSHISTORY WHERE TO_CHAR(TIMESTAMP,'YYYY-MM-DD') BETWEEN ? AND ? AND ACCOUNTNUMBER = ?",this.datefrom,this.dateto,this.accountnumber);
	     ...
	     if (!r.isEmpty())
	     {
	    	 for(DataRow row: r){
		         ...
		         m.setProperty("FEE", row.getString("FEE") == null || row.getString("FEE") == "" ? "" : LongUtil.toString(Long.parseLong(row.getString("FEE").toString())));add(m);
	    	 }
	...
	public boolean hasRowsAll() {
		DataRowCollection r = SystemInfo.getDb().QueryDataRows("SELECT * FROM TBLFAILEDTRANSHISTORY WHERE TO_CHAR(TIMESTAMP,'YYYY-MM-DD') BETWEEN ? AND ?",this.datefrom,this.dateto);
	     ...
	     if (!r.isEmpty())
	     {
	    	 for(DataRow row: r){
		         ...
		         m.setProperty("SOURCEBALANCEAFTER", row.getString("SOURCEBALANCEAFTER") == null ? "" : LongUtil.toString(Long.parseLong(row.getString("SOURCEBALANCEAFTER").toString())));
		         m.setProperty("DISCOUNT", row.getString("DISCOUNT") == null || row.getString("DISCOUNT") == "" ? "" : LongUtil.toString(Long.parseLong(row.getString("DISCOUNT").toString())));
			     m.setProperty("FEE", row.getString("FEE") == null || row.getString("FEE") == "" ? "" : LongUtil.toString(Long.parseLong(row.getString("FEE").toString())));
		         add(m);
	    	 }
