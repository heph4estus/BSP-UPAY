...
public class NewCashierAmlReceiveCommand extends UICommand{
	...
	public IView execute() {
		...
		try {
			...
			if(sess.exists()) {
				...
				if(reg.exist()){
					reg.setState(new ObjectState("01", "Aml Setting already registered"));
				    ...
				    audit.setData(accountnumber+"|"+userslevel+"|"+key+"|"+minamount+"|"+maxamount+"|"+maxamountday+"|"+maxtransweek+"|"+maxamountmonth+"|"+maxtransday+"|"+maxtransweek+"|"+maxtransmonth);
				...
				reg.setState(new ObjectState("00", "Added Succesfully"));
				AuditTrail audit  = new AuditTrail();
	    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
	    		audit.setModuleid(String.valueOf(this.getId()));
	    		audit.setEntityid(accountnumber);
