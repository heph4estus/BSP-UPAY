...
public class LockedUnlockedCommand extends UICommand{
	...
	public IView execute() {
		...
		try {
			...
			if(sess.exists()) {
				...
				String lock = this.params.get("Locked").toString();
				...
				if(!reg.validate()){
					...
					if(reg.lockunlock()){
						reg.setState(new ObjectState("00", "Succesfully "+lock!="" && lock.equals("NO")?"Locked Account":"Unlocked Account"));
						AuditTrail audit  = new AuditTrail();
			    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
			    		audit.setModuleid(String.valueOf(this.getId()));
			    		audit.setEntityid(username);
			    		audit.setLog("Successfully "+lock!="" && lock.equals("NO")?"Locked Account":"Unlocked Account");
			    		audit.setStatus(reg.getState().getCode());
			    		audit.setUserid(reg.getAuthorizedSession().getAccount().getId());
			    		audit.setUsername(reg.getAuthorizedSession().getAccount().getUserName());
			    		audit.setSessionid(reg.getAuthorizedSession().getId());
	...
	public String getKey() {
		return "LOCKUNLOCKBACKOFFICEUSER";
	}
