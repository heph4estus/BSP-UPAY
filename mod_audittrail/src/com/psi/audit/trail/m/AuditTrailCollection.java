...
public class AuditTrailCollection extends ModelCollection{
	...
	public boolean getPrepaidCol() {
		DataRowCollection r = SystemInfo.getDb().QueryDataRows("SELECT A.USERNAME,A.USERID,A.LOG,IP,TIMESTAMP,A.STATUS,M.MODULE,A.SESSIONID,A.ENTITYID,A.DATA,A.REMARKS,A.BROWSER,A.BROWSERVERSION,A.REQUEST,A.OS,A.PORTALVERSION,A.USERSLEVEL,A.DATA2 FROM TBLAUDITTRAIL A INNER JOIN TBLUSERS U ON U.USERNAME = A.USERNAME INNER JOIN TBLUSERSLEVEL UL ON UL.USERSLEVEL = U.USERSLEVEL INNER JOIN TBLMODULE M ON TRIM(TO_CHAR(M.ID,'9999999')) = A.MODULE WHERE UL.PORTAL ='operator' AND U.USERSLEVEL = ? AND TO_CHAR(TIMESTAMP,'YYYY-MM-DD') BETWEEN ? AND ?",this.userslevel,this.datefrom,this.dateto);
