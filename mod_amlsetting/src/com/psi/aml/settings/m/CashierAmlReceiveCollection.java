...
public class CashierAmlReceiveCollection extends ModelCollection{

	protected String accountnumber;
	
	@Override
	public boolean hasRows() {
		DataRowCollection rows = SystemInfo.getDb().QueryDataRows("SELECT * FROM TBLAMLACCOUNTTYPERECEIVE WHERE STATUS=1 AND ACCOUNTNUMBER = ?",this.accountnumber);
		if (!rows.isEmpty())
	     {
	    	 for(DataRow row: rows){	
		         ...
		         m.setProperty("ID", row.getString("ID").toString());
		 	       ...
		 	       m.setProperty("ALERTLEVEL", row.getString("ALERTLEVEL").toString());
		 	      m.setProperty("SCOPE", row.getString("SCOPE").toString());
		 	      add(m);
	    	 }
	     }
	     return rows.size() > 0;
