...
import com.psi.commission.m.AvailableCustomerComCollection;
...
public class AvailableCustomerComColCommand extends UICommand{
	...
	public IView execute() {
		...
		try {
			...
			if(sess.exists()) {
				...
				AvailableCustomerComCollection useraccounts = new AvailableCustomerComCollection();
				...
				if(useraccounts.hasRows()){
		    		...
		    		audit.setLog("Successfully fetched customer commission");
		    		audit.setStatus("00");
					...
					ObjectState state = new ObjectState("01","No customer commission found");
					AuditTrail audit  = new AuditTrail();
		    		audit.setIp(useraccounts.getAuthorizedSession().getIpAddress());
		    		audit.setModuleid(String.valueOf(this.getId()));
		    		audit.setEntityid(service);
		    		audit.setLog("No customer commission found");
		    		audit.setStatus("01");
		    		audit.setUserid(useraccounts.getAuthorizedSession().getAccount().getId());
		    		audit.setUsername(useraccounts.getAuthorizedSession().getAccount().getUserName());
		    		audit.setSessionid(useraccounts.getAuthorizedSession().getId());
	...
	public String getKey() {
		return "AVAILABLECUSTOMERCOMMISSION";
	}
