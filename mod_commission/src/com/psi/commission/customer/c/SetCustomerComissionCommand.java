...
import com.psi.commission.m.ManageCustomerCommission;
...
public class SetCustomerComissionCommand extends UICommand{
	    ...
	    try{
	    	sess = ExistingSession.parse(this.reqHeaders);	    
			  if(sess.exists()) {
					...
					ManageCustomerCommission changestat = new ManageCustomerCommission();
					...
					try {
						...
						if(type.equals("AIRTIME")){
							if(changestat.setrevenueairtime()){
								ObjectState state = new ObjectState("00","Successfully set customer commission");
								changestat.setState(state);
								AuditTrail audit  = new AuditTrail();
					    		audit.setIp(changestat.getAuthorizedSession().getIpAddress());
					    		audit.setModuleid(String.valueOf(this.getId()));
					    		audit.setEntityid(type+"|"+category+"|"+tariff);
							    ...
							    audit.setData(type+"|"+category+"|"+tariff);
					    		audit.insert();
								JsonView view = new JsonView(changestat);
								return view;
							}
							else{
								ObjectState state = new ObjectState("01","Unable to set customer commission");
								changestat.setState(state);
								AuditTrail audit  = new AuditTrail();
					    		audit.setIp(changestat.getAuthorizedSession().getIpAddress());
					    		audit.setModuleid(String.valueOf(this.getId()));
