...
import com.psi.commission.m.AvailableBranchComCollection;
...
public class AvailableBranchComColCommand extends UICommand{
	...
	public IView execute() {
		...
		try {
			...
			if(sess.exists()) {
				...
				AvailableBranchComCollection useraccounts = new AvailableBranchComCollection();
				...
				if(useraccounts.hasRows()){
		    		...
		    		audit.setLog("Successfully fetched provider revenue");
		    		audit.setStatus("00");
		    		...
		    		audit.setModuleid(String.valueOf(this.getId()));
		    		audit.setEntityid(service);
		    		audit.setLog("No provider product found");
		    		audit.setStatus("01");
		    		audit.setUserid(useraccounts.getAuthorizedSession().getAccount().getId());
		    		audit.setUsername(useraccounts.getAuthorizedSession().getAccount().getUserName());
		    		audit.setSessionid(useraccounts.getAuthorizedSession().getId());
