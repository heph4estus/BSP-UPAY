...
import com.psi.commission.m.BranchProductCollection;
...
public class BranchProductColCommand extends UICommand{
	...
	public IView execute() {
		...
		try {
			...
			if(sess.exists()) {
				...
				BranchProductCollection useraccounts = new BranchProductCollection();
				...
				if(useraccounts.hasRows()){
		    		...
		    		audit.setLog("Successfully fetched branch products");
		    		audit.setStatus("00");
				    ...
				    audit.setData(service+"|"+accountnumber+"|"+category);
		    		audit.insert();
		    		...
		    		audit.setModuleid(String.valueOf(this.getId()));
		    		audit.setEntityid(service);
		    		audit.setLog("No branch product found");
		    		audit.setStatus("01");
		    		audit.setUserid(useraccounts.getAuthorizedSession().getAccount().getId());
		    		audit.setUsername(useraccounts.getAuthorizedSession().getAccount().getUserName());
		    		audit.setSessionid(useraccounts.getAuthorizedSession().getId());
