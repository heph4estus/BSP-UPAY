...
public class CustomerProductCollection extends ModelCollection{
	...
	public boolean hasRows() {
		...
		if(!this.category.equals("ALL")){rr = SystemInfo.getDb().QueryDataRow("SELECT * FROM TBLBRANCHPRODUCTS WHERE TYPE='PRODUCTCUS' AND SERVICE = ? AND CATEGORY IS NULL",this.service);}
		else{rr = SystemInfo.getDb().QueryDataRow("SELECT * FROM TBLBRANCHPRODUCTS WHERE TYPE='PRODUCTCUS' AND SERVICE = ? AND CATEGORY = 'ALL'",this.service);}
