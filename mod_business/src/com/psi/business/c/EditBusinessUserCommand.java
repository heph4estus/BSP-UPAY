...
import com.psi.business.m.EditBusinessUser;
...
public class EditBusinessUserCommand extends UICommand{
	...
	public IView execute() {	
		...
		try {
			...
			if(sess.exists()) {
				...
				EditBusinessUser reg = new EditBusinessUser();
				...
				if(reg.exist()){
					if(reg.update()){
						EmailUtils.sendUpdate(email, firstname, lastname,username,reg.getAuthorizedSession().getIpAddress(),reg.getAuthorizedSession().getAccount().getUserName());
						reg.setState(new ObjectState("00", "Succesfully updated business manager user"));
					}else{
					...
					reg.setState(new ObjectState("01", "Account does not exist"));
					AuditTrail audit  = new AuditTrail();
		    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
		    		audit.setModuleid(String.valueOf(this.getId()));
		    		audit.setEntityid(accountnumber);
