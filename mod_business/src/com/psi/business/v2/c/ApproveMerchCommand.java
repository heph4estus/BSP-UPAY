package com.psi.business.v2.c;

import com.psi.audit.trail.m.AuditTrail;
import com.psi.business.util.Business;
import com.psi.business.v.JsonView;
import com.psi.business.v2.m.ManageMerchant;
import com.tlc.common.Logger;
import com.tlc.gui.absmobile.modules.session.m.ExistingSession;
import com.tlc.gui.absmobile.modules.session.v.SessionView;
import com.tlc.gui.modules.common.IView;
import com.tlc.gui.modules.common.ObjectState;
import com.tlc.gui.modules.common.UICommand;
import com.tlc.gui.modules.session.SessionNotFoundException;
import com.tlc.gui.modules.session.UISession;

public class ApproveMerchCommand extends UICommand{

	@Override
	public IView execute() {
		ExistingSession sess = null;
		SessionView v = null;
		
		try {
			sess = ExistingSession.parse(this.reqHeaders);		
			if(sess.exists()) {
				String accountnumber = this.params.get("AccountNumber");
				String password = this.params.get("Password");
				 	
				ManageMerchant reg = new ManageMerchant();
				
				reg.setAccountnumber(accountnumber);
				reg.setPassword(password);
				reg.setAuthorizedSession(sess);
				reg.mechantdetails();
						if(!reg.validate()){
							reg.setState(new ObjectState("02", "Incorrect Password"));
//							AuditTrail audit  = new AuditTrail();
//				    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
//				    		audit.setModuleid(String.valueOf(this.getId()));
//				    		audit.setEntityid(accountnumber);
//				    		audit.setLog(reg.getState().getMessage());
//				    		audit.setStatus(reg.getState().getCode());
//				    		audit.setUserid(reg.getAuthorizedSession().getAccount().getId());
//				    		audit.setUsername(reg.getAuthorizedSession().getAccount().getUserName());
//				    		audit.setSessionid(reg.getAuthorizedSession().getId());
//				    		audit.setBrowser(reg.getAuthorizedSession().getBrowser());
//						    audit.setBrowserversion(reg.getAuthorizedSession().getBrowserversion());
//						    audit.setPortalversion(reg.getAuthorizedSession().getPortalverion());
//						    audit.setOs(reg.getAuthorizedSession().getOs());
//						    audit.setUserslevel(reg.getAuthorizedSession().getAccount().getGroup().getName());
//						    audit.setRequest(this.params.toString());
////						    audit.setData(reg.getBusinessname());
//						    audit.setData(reg.toString());
							AuditTrail audit = createAudit(reg);
				    		audit.insert();
							return new JsonView(reg);
						}
						if(!reg.existpndg()){
							reg.setState(new ObjectState("01", "Merchant account do not exist"));
//							AuditTrail audit  = new AuditTrail();
//				    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
//				    		audit.setModuleid(String.valueOf(this.getId()));
//				    		audit.setEntityid(accountnumber);
//				    		audit.setLog(reg.getState().getMessage());
//				    		audit.setStatus(reg.getState().getCode());
//				    		audit.setUserid(reg.getAuthorizedSession().getAccount().getId());
//				    		audit.setUsername(reg.getAuthorizedSession().getAccount().getUserName());
//				    		audit.setSessionid(reg.getAuthorizedSession().getId());
//				    		audit.setBrowser(reg.getAuthorizedSession().getBrowser());
//						    audit.setBrowserversion(reg.getAuthorizedSession().getBrowserversion());
//						    audit.setPortalversion(reg.getAuthorizedSession().getPortalverion());
//						    audit.setOs(reg.getAuthorizedSession().getOs());
//						    audit.setUserslevel(reg.getAuthorizedSession().getAccount().getGroup().getName());
//						    audit.setRequest(this.params.toString());
////						    audit.setData(reg.getBusinessname());
//						    audit.setData(reg.toString());
							AuditTrail audit = createAudit(reg);
				    		audit.insert();
							return new JsonView(reg);
						}
						if(reg.existbusiness()){
							reg.setState(new ObjectState("03", "Merchant account name already exists"));
//							AuditTrail audit  = new AuditTrail();
//				    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
//				    		audit.setModuleid(String.valueOf(this.getId()));
//				    		audit.setEntityid(accountnumber);
//				    		audit.setLog(reg.getState().getMessage());
//				    		audit.setStatus(reg.getState().getCode());
//				    		audit.setUserid(reg.getAuthorizedSession().getAccount().getId());
//				    		audit.setUsername(reg.getAuthorizedSession().getAccount().getUserName());
//				    		audit.setSessionid(reg.getAuthorizedSession().getId());
//				    		audit.setBrowser(reg.getAuthorizedSession().getBrowser());
//						    audit.setBrowserversion(reg.getAuthorizedSession().getBrowserversion());
//						    audit.setPortalversion(reg.getAuthorizedSession().getPortalverion());
//						    audit.setOs(reg.getAuthorizedSession().getOs());
//						    audit.setUserslevel(reg.getAuthorizedSession().getAccount().getGroup().getName());
//						    audit.setRequest(this.params.toString());
////						    audit.setData(reg.getBusinessname());
//						    audit.setData(reg.toString());
							AuditTrail audit = createAudit(reg);
				    		audit.insert();
							return new JsonView(reg);
						}
						if(reg.existmsisdn()){
							reg.setState(new ObjectState("04", "Primary Mobile Number already registered"));
//							AuditTrail audit  = new AuditTrail();
//				    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
//				    		audit.setModuleid(String.valueOf(this.getId()));
//				    		audit.setEntityid(accountnumber);
//				    		audit.setLog(reg.getState().getMessage());
//				    		audit.setStatus(reg.getState().getCode());
//				    		audit.setUserid(reg.getAuthorizedSession().getAccount().getId());
//				    		audit.setUsername(reg.getAuthorizedSession().getAccount().getUserName());
//				    		audit.setSessionid(reg.getAuthorizedSession().getId());
//				    		audit.setBrowser(reg.getAuthorizedSession().getBrowser());
//						    audit.setBrowserversion(reg.getAuthorizedSession().getBrowserversion());
//						    audit.setPortalversion(reg.getAuthorizedSession().getPortalverion());
//						    audit.setOs(reg.getAuthorizedSession().getOs());
//						    audit.setUserslevel(reg.getAuthorizedSession().getAccount().getGroup().getName());
//						    audit.setRequest(this.params.toString());
////						    audit.setData(reg.getBusinessname());
//						    audit.setData(reg.toString());
							AuditTrail audit = createAudit(reg);
				    		audit.insert();
							return new JsonView(reg);
						}
						if(reg.existauthmsisdn()){
							reg.setState(new ObjectState("05", "Authorized Mobile Number already exist"));
//							AuditTrail audit  = new AuditTrail();
//				    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
//				    		audit.setModuleid(String.valueOf(this.getId()));
//				    		audit.setEntityid(accountnumber);
//				    		audit.setLog(reg.getState().getMessage());
//				    		audit.setStatus(reg.getState().getCode());
//				    		audit.setUserid(reg.getAuthorizedSession().getAccount().getId());
//				    		audit.setUsername(reg.getAuthorizedSession().getAccount().getUserName());
//				    		audit.setSessionid(reg.getAuthorizedSession().getId());
//				    		audit.setBrowser(reg.getAuthorizedSession().getBrowser());
//						    audit.setBrowserversion(reg.getAuthorizedSession().getBrowserversion());
//						    audit.setPortalversion(reg.getAuthorizedSession().getPortalverion());
//						    audit.setOs(reg.getAuthorizedSession().getOs());
//						    audit.setUserslevel(reg.getAuthorizedSession().getAccount().getGroup().getName());
//						    audit.setRequest(this.params.toString());
////						    audit.setData(reg.getBusinessname());
//						    audit.setData(reg.toString());
							AuditTrail audit = createAudit(reg);
				    		audit.insert();
							return new JsonView(reg);
						}
						if(reg.existauthemail()){
							reg.setState(new ObjectState("06", "Authorized Email Address already exist"));
//							AuditTrail audit  = new AuditTrail();
//				    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
//				    		audit.setModuleid(String.valueOf(this.getId()));
//				    		audit.setEntityid(accountnumber);
//				    		audit.setLog(reg.getState().getMessage());
//				    		audit.setStatus(reg.getState().getCode());
//				    		audit.setUserid(reg.getAuthorizedSession().getAccount().getId());
//				    		audit.setUsername(reg.getAuthorizedSession().getAccount().getUserName());
//				    		audit.setSessionid(reg.getAuthorizedSession().getId());
//				    		audit.setBrowser(reg.getAuthorizedSession().getBrowser());
//						    audit.setBrowserversion(reg.getAuthorizedSession().getBrowserversion());
//						    audit.setPortalversion(reg.getAuthorizedSession().getPortalverion());
//						    audit.setOs(reg.getAuthorizedSession().getOs());
//						    audit.setUserslevel(reg.getAuthorizedSession().getAccount().getGroup().getName());
//						    audit.setRequest(this.params.toString());
////						    audit.setData(reg.getBusinessname());
//						    audit.setData(reg.toString());
							AuditTrail audit = createAudit(reg);
				    		audit.insert();
							return new JsonView(reg);
						}
						if(reg.existauthusername()){
							reg.setState(new ObjectState("07", "User Name already exist"));
//							AuditTrail audit  = new AuditTrail();
//				    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
//				    		audit.setModuleid(String.valueOf(this.getId()));
//				    		audit.setEntityid(accountnumber);
//				    		audit.setLog(reg.getState().getMessage());
//				    		audit.setStatus(reg.getState().getCode());
//				    		audit.setUserid(reg.getAuthorizedSession().getAccount().getId());
//				    		audit.setUsername(reg.getAuthorizedSession().getAccount().getUserName());
//				    		audit.setSessionid(reg.getAuthorizedSession().getId());
//				    		audit.setBrowser(reg.getAuthorizedSession().getBrowser());
//						    audit.setBrowserversion(reg.getAuthorizedSession().getBrowserversion());
//						    audit.setPortalversion(reg.getAuthorizedSession().getPortalverion());
//						    audit.setOs(reg.getAuthorizedSession().getOs());
//						    audit.setUserslevel(reg.getAuthorizedSession().getAccount().getGroup().getName());
//						    audit.setRequest(this.params.toString());
////						    audit.setData(reg.getBusinessname());
//						    audit.setData(reg.toString());
							AuditTrail audit = createAudit(reg);
				    		audit.insert();
							return new JsonView(reg);
						}
						if(reg.approve()){
							reg.setState(new ObjectState("00", "Successfully approved merchant account"));
//							AuditTrail audit  = new AuditTrail();
//				    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
//				    		audit.setModuleid(String.valueOf(this.getId()));
//				    		audit.setEntityid(accountnumber);
//				    		audit.setLog(reg.getState().getMessage());
//				    		audit.setStatus(reg.getState().getCode());
//				    		audit.setUserid(reg.getAuthorizedSession().getAccount().getId());
//				    		audit.setUsername(reg.getAuthorizedSession().getAccount().getUserName());
//				    		audit.setSessionid(reg.getAuthorizedSession().getId());
//				    		audit.setBrowser(reg.getAuthorizedSession().getBrowser());
//						    audit.setBrowserversion(reg.getAuthorizedSession().getBrowserversion());
//						    audit.setPortalversion(reg.getAuthorizedSession().getPortalverion());
//						    audit.setOs(reg.getAuthorizedSession().getOs());
//						    audit.setUserslevel(reg.getAuthorizedSession().getAccount().getGroup().getName());
//						    audit.setRequest(this.params.toString());
//						    audit.setData(reg.getBusinessname());
							AuditTrail audit = createAudit(reg);
				    		audit.insert();
							return new JsonView(reg);
						}else{
							reg.setState(new ObjectState("01", "Failed! Unable to approve merchant account"));
//							AuditTrail audit  = new AuditTrail();
//				    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
//				    		audit.setModuleid(String.valueOf(this.getId()));
//				    		audit.setEntityid(accountnumber);
//				    		audit.setLog(reg.getState().getMessage());
//				    		audit.setStatus(reg.getState().getCode());
//				    		audit.setUserid(reg.getAuthorizedSession().getAccount().getId());
//				    		audit.setUsername(reg.getAuthorizedSession().getAccount().getUserName());
//				    		audit.setSessionid(reg.getAuthorizedSession().getId());
//				    		audit.setBrowser(reg.getAuthorizedSession().getBrowser());
//						    audit.setBrowserversion(reg.getAuthorizedSession().getBrowserversion());
//						    audit.setPortalversion(reg.getAuthorizedSession().getPortalverion());
//						    audit.setOs(reg.getAuthorizedSession().getOs());
//						    audit.setUserslevel(reg.getAuthorizedSession().getAccount().getGroup().getName());
//						    audit.setRequest(this.params.toString());
//						    audit.setData(reg.getBusinessname());
							AuditTrail audit = createAudit(reg);
				    		audit.insert();
							return new JsonView(reg);
						}
			}else{		
				UISession u = new UISession(null);
			    u.setState(new ObjectState("TLC-3902-01"));
			    v = new SessionView(u);
			}
		}catch (SessionNotFoundException e) {
			UISession u = new UISession(null);
		    u.setState(new ObjectState("TLC-3902-01"));
		    v = new SessionView(u);
			Logger.LogServer(e);
	} catch (Exception e) {
		Logger.LogServer(e);
	}return v;
	}

	@Override
	public String getKey() {
		return "APPROVEMERCHANT";
	}

	@Override
	public int getId() {
		return 1108;
	}
	
	private AuditTrail createAudit(ManageMerchant reg) {
		AuditTrail audit = new AuditTrail();
		audit.setIp(reg.getAuthorizedSession().getIpAddress());
		audit.setModuleid(String.valueOf(this.getId()));
		audit.setEntityid(reg.getAccountnumber());
		audit.setLog(reg.getState().getMessage());
		audit.setStatus(reg.getState().getCode());
		audit.setUserid(reg.getAuthorizedSession().getAccount().getId());
		audit.setUsername(reg.getAuthorizedSession().getAccount().getUserName());
		audit.setSessionid(reg.getAuthorizedSession().getId());
		audit.setBrowser(reg.getAuthorizedSession().getBrowser());
	    audit.setBrowserversion(reg.getAuthorizedSession().getBrowserversion());
	    audit.setPortalversion(reg.getAuthorizedSession().getPortalverion());
	    audit.setOs(reg.getAuthorizedSession().getOs());
	    audit.setUserslevel(reg.getAuthorizedSession().getAccount().getGroup().getName());
	    audit.setRequest(this.params.toString());
	    audit.setData2(reg.toString());
		return audit;
	}
}
