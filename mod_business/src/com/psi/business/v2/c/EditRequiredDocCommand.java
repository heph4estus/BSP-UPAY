...
import com.psi.business.v2.m.ManageRequiredDocuments;
...
public class EditRequiredDocCommand extends UICommand{
	...
	public IView execute() {
		...
		try {
			...
			if(sess.exists()) {
				...
				String image = this.params.get("ImgProof").toString();		
				String status = this.params.get("Status").toString();
				
				ManageRequiredDocuments reg = new ManageRequiredDocuments();
			    ...
			    reg.setAuthorizedSession(sess);
					...
					if(reg.update()){
						reg.setState(new ObjectState("00", "Successfully updated required documents"));
						...
						reg.setState(new ObjectState("99", "Failed! Unable to update required documents"));
						AuditTrail audit  = new AuditTrail();
						audit.setIp(reg.getAuthorizedSession().getIpAddress());
						audit.setModuleid(String.valueOf(this.getId()));
						audit.setEntityid(accountnumber);
