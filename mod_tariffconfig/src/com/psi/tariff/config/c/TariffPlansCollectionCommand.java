...
import com.psi.tariff.config.m.TariffPlansCollection;
...
public class TariffPlansCollectionCommand extends UICommand{
	...
	public IView execute() {
		...
		try {
			...
			if(sess.exists()) {
				...
				TariffPlansCollection model = new TariffPlansCollection();
					...
					model.setAuthorizedSession(sess);
						...
						if(model.hasRows()){
				    		...
				    		audit.setEntityid("");
				    		audit.setLog("Successfully fetched tariff plans data");
				    		audit.setStatus("00");
							...
							return new CollectionView("00",model);  
								...
								ObjectState state = new ObjectState("01", "No tariff plans found");
								AuditTrail audit  = new AuditTrail();
					    		audit.setIp(model.getAuthorizedSession().getIpAddress());
					    		audit.setModuleid(String.valueOf(this.getId()));
					    		audit.setEntityid("");
