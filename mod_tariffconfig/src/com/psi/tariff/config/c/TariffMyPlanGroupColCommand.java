...
import com.psi.tariff.config.m.TariffMyPlanGroupCollection;
import com.psi.tariff.config.v.CollectionView;
import com.psi.tariff.config.v.NoDataFoundView;
...
public class TariffMyPlanGroupColCommand extends UICommand{
	...
	public IView execute() {
		...
		try {
			...
			if(sess.exists()) {
				...
				String groupname = this.params.get("GroupName").toString();
				String type = this.params.get("Type").toString();
				TariffMyPlanGroupCollection model = new TariffMyPlanGroupCollection(groupname);
					model.setType(type);
					model.setAuthorizedSession(sess);
						...
						if(model.hasRows()){
				    		...
				    		audit.setEntityid("");
				    		audit.setLog("Successfully fetched group tariff plans data");
				    		audit.setStatus("00");
						    ...
						    audit.setData(groupname+"|"+type);
				    		audit.insert();
							return new CollectionView("00",model);  
						}else{
								ObjectState state = new ObjectState("01", "No group tariff plans found");
								AuditTrail audit  = new AuditTrail();
					    		audit.setIp(model.getAuthorizedSession().getIpAddress());
					    		audit.setModuleid(String.valueOf(this.getId()));
					    		audit.setEntityid("");
	...
	public String getKey() {
		return "GROUPTARIFFMYPLANSCOLLECTION";
	}
