...
import com.psi.tariff.config.m.TariffPlansSpeCollection;
...
public class TariffPlansSpeColCommand extends UICommand{
	...
	public IView execute() {
		...
		try {
			...
			if(sess.exists()) {
				...
				TariffPlansSpeCollection model = new TariffPlansSpeCollection();
					model.setType(type);
					model.setAuthorizedSession(sess);
						...
						if(model.hasRows()){
				    		...
				    		audit.setEntityid("");
				    		audit.setLog("Successfully fetched specific tariff plans data");
				    		audit.setStatus("00");
							...
							return new CollectionView("00",model);  
								...
								ObjectState state = new ObjectState("01", "No specific tariff plans found");
								AuditTrail audit  = new AuditTrail();
					    		audit.setIp(model.getAuthorizedSession().getIpAddress());
					    		audit.setModuleid(String.valueOf(this.getId()));
					    		audit.setEntityid("");
	...
	public String getKey() {
		return "SPETARIFFPLANSCOLLECTION";
	}
