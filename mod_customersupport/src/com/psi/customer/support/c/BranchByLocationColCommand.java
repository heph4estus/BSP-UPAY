...
import com.psi.customer.support.m.BranchByLocationCollection;
...
public class BranchByLocationColCommand extends UICommand{
	...
	public IView execute() {
		...
		String region = this.params.get("Region").toString();
		String city = this.params.get("City").toString();
		BranchByLocationCollection model = new BranchByLocationCollection();
					...
					if(model.hasRows()){
			    		...
			    		audit.setEntityid(region+"|"+city);
			    		audit.setLog("Successfully fetched branch by location data");
			    		audit.setStatus("00");
						...
						return new CollectionView("00",model);  
							...
							ObjectState state = new ObjectState("01", "No branch by location data found");
							AuditTrail audit  = new AuditTrail();
				    		audit.setIp(model.getAuthorizedSession().getIpAddress());
				    		audit.setModuleid(String.valueOf(this.getId()));
				    		audit.setEntityid(region+"|"+city);
				    		audit.setLog("No branch by location data found");
				    		audit.setStatus("01");
				    		audit.setUserid(model.getAuthorizedSession().getAccount().getId());
				    		audit.setUsername(model.getAuthorizedSession().getAccount().getUserName());
				    		audit.setSessionid(model.getAuthorizedSession().getId());
	...
	public String getKey() {
		return "BRANCHBYLOCATIONCOLLECTION";
	}
