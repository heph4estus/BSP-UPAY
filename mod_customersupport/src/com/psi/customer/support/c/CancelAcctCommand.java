...
public class CancelAcctCommand extends UICommand{
			...
			if(sess.exists()) {
				...
				reg.setAuthorizedSession(sess);
					...
					if(accounttype.equals("BRANCH")){
						if(reg.cancelbranch()){
							reg.setState(new ObjectState("00", "Successfully cancelled branch account"));
							AuditTrail audit  = new AuditTrail();
				    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
				    		audit.setModuleid(String.valueOf(this.getId()));
				    		audit.setEntityid(accountnumber);
						...
						if(reg.cancelcompany()){
							reg.setState(new ObjectState("00", "Successfully cancelled merchant account"));
							AuditTrail audit  = new AuditTrail();
				    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
				    		audit.setModuleid(String.valueOf(this.getId()));
				    		audit.setEntityid(accountnumber);
						...
						if(reg.cancelcustomer()){
							reg.setState(new ObjectState("00", "Successfully cancelled customer account"));
							AuditTrail audit  = new AuditTrail();
				    		audit.setIp(reg.getAuthorizedSession().getIpAddress());
				    		audit.setModuleid(String.valueOf(this.getId()));
				    		audit.setEntityid(accountnumber);
