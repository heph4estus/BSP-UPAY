...
public class CustomerCollection extends ModelCollection {
	
protected String accountnumber;
	@Override
	public boolean hasRows() {		
	     DataRowCollection r = SystemInfo.getDb().QueryDataRows("SELECT ACCOUNTNUMBER,DECRYPT(FQN,?,ACCOUNTNUMBER) FQN,DECRYPT(TYPE,?,ACCOUNTNUMBER) TYPE,MSISDN,STATUS,LOCKED,EMAIL,REGDATE,LASTNAME,FIRSTNAME,MIDDLENAME,CITY,SPECIFICADDRESS,POSTALCODE FROM ADMDBMC.TBLACCOUNTINFO WHERE ACCOUNTNUMBER = ? ORDER BY DECRYPT(FQN,?,ACCOUNTNUMBER) ASC",SystemInfo.getDb().getCrypt(),SystemInfo.getDb().getCrypt(),this.accountnumber,SystemInfo.getDb().getCrypt());
