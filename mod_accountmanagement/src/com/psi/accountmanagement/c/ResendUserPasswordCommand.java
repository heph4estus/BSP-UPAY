...
public class ResendUserPasswordCommand extends UICommand {
	...
	public IView execute() {
		...
		try {
			...
			if(sess.exists()) {
				...
				if(mgtacct.resenduserpassword(userid)){
					mgtacct.setState(new ObjectState("00", "Resend Password Successfully"));
					AuditTrail audit  = new AuditTrail();
		    		audit.setIp(mgtacct.getAuthorizedSession().getIpAddress());
		    		audit.setModuleid(String.valueOf(this.getId()));
		    		audit.setEntityid(userid);
		    		audit.setLog("Resend Username/Password Successfully");
		    		audit.setStatus("00");
					...
					mgtacct.setState(new ObjectState("01", "Failed! Unable to resend username/password"));
					AuditTrail audit  = new AuditTrail();
		    		audit.setIp(mgtacct.getAuthorizedSession().getIpAddress());
		    		audit.setModuleid(String.valueOf(this.getId()));
		    		audit.setEntityid(userid);
		    		audit.setLog("Failed! Unable to resend username/password");
		    		audit.setStatus("01");
		    		audit.setUserid(mgtacct.getAuthorizedSession().getAccount().getId());
		    		audit.setUsername(mgtacct.getAuthorizedSession().getAccount().getUserName());
		    		audit.setSessionid(mgtacct.getAuthorizedSession().getId());
