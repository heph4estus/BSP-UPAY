...
import com.psi.subs.management.m.FileUploadedCollection;
...
public class FileUploadedCommand extends UICommand{
	...
	public IView execute() {
		...
		try {
			...
			if(sess.exists()) {	
				FileUploadedCollection model = new FileUploadedCollection();
				...
				if(model.hasRows()){
		    		...
		    		audit.setEntityid("");
		    		audit.setLog("Successfully fetch file uploaded data");
		    		audit.setStatus("00");
		    		...
		    		audit.setModuleid(String.valueOf(this.getId()));
		    		audit.setEntityid("");
		    		audit.setLog("No file uploaded data found");
		    		audit.setStatus("01");
		    		audit.setUserid(model.getAuthorizedSession().getAccount().getId());
		    		audit.setUsername(model.getAuthorizedSession().getAccount().getUserName());
		    		audit.setBrowser(model.getAuthorizedSession().getBrowser());
